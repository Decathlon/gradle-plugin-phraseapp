...
class ReducerHelper(val platform: Platform) {
     ...
     */
    fun reduceKeysForAllStringsFilesAndForAllLocales(strings: Map<String, ResourceTranslation>, remoteStrings: Map<String, LocaleContent>): Map<String, Map<ResFolderType, Resource>> =
     ...
     */
    private fun reduceKeysForAllLocales(stringsFile: ResourceTranslation, remoteStrings: Map<String, LocaleContent>): Map<ResFolderType, Resource> {
    ...
    private fun reduceKeys(keys: Set<String>, content: LocaleContent): Resource {
        ...
        return Resource(resource.strings.filter { intersect.contains(it.key) } + resource.plurals.filter { intersect.contains(it.key) })
    }
}